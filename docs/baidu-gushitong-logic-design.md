...
- 全球指数榜单：
  - 接口：`/vapi/v1/globalindexrank`
  - 按 `area` 分区拉取，例如 `america / asia / euro_africa`
- 外汇：
  - 接口：`/api/getbanner`
  - 参数：`market=foreign&finClientType=pc`

当前本地代码的行为是：

1. 先根据 watchlist 把百度项目按 `area` 分组。
2. 对每个分组发 HTTP 请求。
3. 指数类用 `BaiduGlobalIndexResponse` 解。
4. 外汇类用 `BaiduBannerResponse` 解。
5. 将返回的 `code -> quote` 映射成 `QuoteSnapshot`。

这部分需要结合现在的新前提一起看：

- 以上是当前旧实现的工作方式。
- 但在新的 `BDUSS-only` 方案下，百度不应该再继续按 `亚洲 / 欧洲 / 美股 / 外汇` 这种区域分组来设计主流程。
- 新流程的主键应该改成：
  - 百度“我的自选”返回的成员列表
  - 每个成员是否被 WSS 覆盖
  - 每轮 HTTP 还剩下哪些项目必须刷新
...
先按 `financeType / subType / market / area` 分类。
...
1. 远端 `marketStatus` 作为动态真相。
2. 本地 `IndexMarketSchedule` 作为兜底真相。
...
- 百度主流程不再需要先区分 `亚洲 / 欧洲 / 美股 / 外汇`
- `area` 最多只保留给旧 watch item 兼容、展示名称或历史迁移使用
- 新调度应该围绕“自选成员”和“transport 覆盖关系”来组织
...
- 开盘时间接口目前还牵涉 `acs-token`
- 你不想去算它

那这里最稳妥的策略不是硬攻，而是把“开盘判断”从核心依赖降成调度优化：
